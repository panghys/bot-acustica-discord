...
async def generartts(ctx,*,texto):
    ...
    archivo = os.path.join("temp_tts", f"tts_{ctx.author}_{texto}.mp3")
    if(archivo):
        if select == True:
...
# hace que se mezclen 2 audios (al mismo tiempo)

@bot.command()
async def mezclar(ctx, link1, link2):
    a1 = await subir_audios.subirAudio(link1, "a1.mp3")
    a2 = await subir_audios.subirAudio(link2, "a2.mp3")

    audio1 = AudioSegment.from_file("a1.mp3")
    audio2 = AudioSegment.from_file("a2.mp3")
    ...
    await ctx.send(file=discord.File("mix.mp3"))

    os.remove(a1); os.remove(a2); os.remove("mix.mp3")


# envia un espectrograma del audio indicado
@bot.command()
async def espectro(ctx, link):


    print("recibido spectro")
    if ctx.author == bot.user:
        return

    await ctx.send("Generando espectrograma...")
    ...
    frecuencia = audio.frame_rate

    nombreImagen = "espectrograma.png"
    plt.figure(figsize=(10, 4))
    plt.specgram(muestras, Fs=frecuencia, NFFT=1024, noverlap=512)
    plt.xlabel("S")
    plt.ylabel("Hz")
    ...
    await ctx.send(file=discord.File(nombreImagen))
    os.remove(archivo)
